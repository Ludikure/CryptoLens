...
- **Phase 1 — shared analysis brain on the Worker** 🚧 ~90% done:
  ...
  - **NEXT STEP:** `POST /full-analysis?symbol=` in `index.ts` (candles → `fetchAllTimeframes` → `computeFullIndicators` per TF → enrichment the worker already fetches in cron [derivatives/sentiment/macro/stockInfo/economic events/cross-asset] → attach ML overlay from the `/ml-predict` path onto the daily indicator → KV read `prompt:<symbol>` for prevState → `buildUserPrompt` → LLM via `aiGatewayURL` → `parseSetups` → register setups in `trade_outcomes` D1 → KV write newState → return AnalysisResult JSON). Then a captured-Swift prompt-text parity test (run the iOS app's buildUserPrompt for a fixture symbol, snapshot to test/fixtures, assert byte-equality). NOTE on parity caveats already known: per-TF MACD line/signal scalars come from the series last values; Active Trade State + archetype track record + outcome history are passed IN (D1) not computed; stock display extras (smaCross/gap/addv) optional until /full-analysis computes them; ET date strings via Intl may differ in spacing from Swift DateFormatter — reconcile against the captured fixture.
