...
| `/basis` | GET | required | Cash-and-carry monitor (2026-08-23): live Coinbase dated nano-futures basis vs spot, annualized, net of fees, with liquidity gating and the margin-call buffer. **READ-ONLY — public market data, no orders, no trade-enabled credentials.** `?fee=` overrides the per-side assumption (default 0.001 = futures legs only, i.e. the COVERED form against BTC already held; buying the spot leg costs ~0.40-0.60%/side and consumes the entire edge). See `docs/research/funding-carry.md` |
