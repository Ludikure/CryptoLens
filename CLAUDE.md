...
- **The 70 gate could go universe-wide unreachable.** `applyCalibration` clamps at the live curve's top bucket, which sat at ~69 — so a hard 70 would silently kill both windows while the 65 notify threshold kept launching analyses free to decline. The gate is now floored at `max(65, min(70, ceiling))` via a new `calibrationCeiling` threaded from `fetchMlCalibration`; with no fitted curve it falls back to 70 on raw.
