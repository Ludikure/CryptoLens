...
- **Phase 1 — shared analysis brain on the Worker** 🚧 ~80% done:
  ...
  - `src/prompt.ts` 🚧 — `systemPrompt` (byte-extracted to `prompt-system.json` via `scripts/extract_system_prompt.py`), `classifyArchetype`, `useTighterBands`, `parseSetups`, `formatPrice` DONE (commits b76b2f8 + the next). **`buildUserPrompt` NOT done — the ~2,090-line pre-computed-flags core. A full ~40-section PORT ROADMAP + the state-design decision is embedded as a comment block in `src/prompt.ts` (just above where buildUserPrompt goes) — follow it.**
  - **NEXT STEP:** port `buildUserPrompt` (AnalysisPrompt.swift 551–2640) → `prompt.ts` following the inline roadmap. KEY: it's STATEFUL — iOS uses UserDefaults `regime_<symbol>` + `killDur_<symbol>`; the worker replicates via KV (builder takes prevState in, returns {prompt, newState}; /full-analysis does the KV I/O). Port only the treatment branch (isTreatment always true). Then `POST /full-analysis` (candles → indicators-full → enrichment → prompt → LLM via gateway → parseSetups → register outcome) + a prompt-text parity test (capture Swift output for a fixture symbol).
