...
- **#1 Volatility pricing** (commit e8d78fa, worker-only): the tradeable expression of the vol edge. New `fetchImpliedVol` (Deribit DVOL — public 30d annualized IV, no auth, US-accessible for DATA even though Deribit doesn't serve US trading) → `runFullAnalysisCore` (BTC/ETH only) compares the app's HAR-RV 24h move FORECAST vs the options-implied move → `VOLATILITY PRICING` prompt line: cheap vol (forecast > priced → long-gamma/straddle favorable, profits either direction) vs rich vol (move already priced). Direction-agnostic. **Caveat surfaced in-prompt: US retail crypto-options access is limited (Deribit US-restricted for trading) — the read is risk/opportunity context; a US-tradeable options venue is the open question for actually executing it.**
