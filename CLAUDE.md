...
- **Phase 2 — React web app** (Cloudflare Pages, full parity) 🚧 **v1 BUILT (commit a9c048e)** in `web/` — Vite + React 18 + TS + lightweight-charts thin client over the Worker. `src/api.ts` mirrors the iOS auth flow (localStorage device_id + token, `X-App-ID`/`X-Device-ID`/`X-Auth-Token`, 401→rotate+re-register); `GET /indicators` + `POST /full-analysis`. Components: ChartPanel (candles + EMA20/50/200 + S/R + Entry/SL/TP price lines), IndicatorTable (Daily/4H/1H), AnalysisView (ML/bias card + setups + analysis markdown via `marked`), App (symbol load + quick-picks + Run AI Analysis). `npm run build` green (tsc strict + vite, 111KB gzip); `npm run dev` → localhost:5173 against the live Worker. **Not yet deployed to Pages** (outward-facing — `npx wrangler pages deploy dist --project-name marketscope-web`). Remaining for full parity: outcome dashboard / ML-calibration / direction-scoreboard cards, indicator sub-panels (RSI/MACD/Stoch/ADX/Vol), watchlist, settings, alerts. **Pending Worker redeploy** to push the sentiment+crossAsset enrichment (live ver 7aa75df1 has /indicators + /full-analysis + derivatives/spot/macro; sentiment+crossAsset committed but undeployed).
