...
- **Phase 1 — shared analysis brain on the Worker** 🚧 ~90% done:
  ...
  - `POST /full-analysis?symbol=` ✅ BUILT (commit 7b06775, **not yet deployed/smoke-tested** — deploy is outward-facing, needs user go-ahead + a Claude round-trip). Pipeline: candles (`fetchAllTimeframes`) → `computeFullIndicators` per TF → ML overlay from cron-cached `ml_preds:all` (win/persistence/pUp/meta/q75/confident onto daily) → outcome-history from `trade_outcomes` D1 → KV-backed prevState (`prompt:<symbol>`, 7d TTL) → `buildUserPrompt` → persist newState → `systemPrompt` → Claude via `aiGatewayURL` → `parseSetups` → return `{analysis, setups, ml, bias}`. Auth-gated + rate-limited (`analyze:<device>` 30/h). Builds clean (wrangler dry-run), 368/368 tests green.
  - **NEXT STEP (3 remaining for Phase 1):** (1) **Layer in the rest of the enrichment** into `/full-analysis` — derivatives/positioning/sentiment/macro/stockInfo/stockSentiment/economic-events/cross-asset/spot-pressure/weekly/spy. The cron (`computeSymbolPredictions`) already fetches much of this; build the `PromptInput` enrichment objects from it (all optional in the builder, so this is additive). (2) **Captured-Swift prompt-parity test** — run the iOS app's `buildUserPrompt` for a fixture symbol, snapshot to `test/fixtures/`, assert byte-equality (the byte-parity gate). (3) **Deploy + smoke-test** `/full-analysis` against the live worker with a throwaway device-id (clean it up; never register the user's real device-id). Known parity caveats to reconcile against the captured fixture: per-TF MACD line/signal scalars come from series-last values; Active Trade State + archetype track record + outcome history are passed IN (D1) not computed; stock display extras (smaCross/gap/addv) optional until computed; ET date strings via Intl may differ in spacing from Swift DateFormatter.
