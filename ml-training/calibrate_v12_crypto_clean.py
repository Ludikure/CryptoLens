...
DOWNLOADS = '/Users/bojanmihovilovic/CryptoLens/ml-training/csv_exports_v11_fixed'  # LEAK-FIXED clean regen (daily in-progress candle dropped)
...
def export_model(market, model, n_samples, x_cal, y_cal, is_lgb):
    ...
    m = {
        ...
        'description': f'v12-CLEAN {market} ({model_type}) — goodR = fwdMaxFavR>=1.5, {n_samples} bars',
    }
    # Write to worker
...
if __name__ == '__main__':
    print("=" * 60)
    print("v11 crypto — LightGBM d4 t150 on /tmp/retrain_crypto regen")
    print("=" * 60)

    # Add POL (post-rename ticker for MATIC); backtest covered both, train on both.
    if 'POL' not in CRYPTO_SYMBOLS:
        CRYPTO_SYMBOLS.append('POL')
    # OP was in the backtest list too but missing from v9 script
    if 'OP' not in CRYPTO_SYMBOLS:
        CRYPTO_SYMBOLS.append('OP')
