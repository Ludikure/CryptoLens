#!/usr/bin/env python3
"""T23 — the T22 pruning test re-run against the PRODUCTION target.

T22 tested y_crash (P(10% drawdown in 10 days)). The shipped model trains on
goodR = fwdMaxFavR >= 1.5 within 24h. Identical arms, folds, purge and ship bar; ONLY the target
changes. BTC is reported separately because T22 showed it as the single meaningful loss (-0.046).
"""
    ...
    for arm,cols in ARMS.items():
        for sym in ASSETS:
            ...
            for i,s in enumerate(starts):
                ...
                m.fit(tr[cols],tr['y_good']); t=te.copy(); t['p']=m.predict_proba(t[cols])[:,1]; P.append(t[['p','y_good']])
            if P:
                D=pd.concat(P); res[arm][sym]=roc_auc_score(D.y_good,D.p)
        print(f"  {arm:<26} mean AUC {np.mean(list(res[arm].values())):.4f}",flush=True)

    print("\n"+"="*92); print("T23 — AUC by asset (LOSO, identical folds)")
